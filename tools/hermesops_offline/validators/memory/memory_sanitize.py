...
def main():
    ...
    args = ap.parse_args()
    result = {'tool': 'memory_sanitize', 'dry_run': args.dry_run}
    result.update({'status': 'sanitized' if args.input else 'dry-run', 'input': args.input, 'blocked_patterns': list(BANNED)})
    if args.report:

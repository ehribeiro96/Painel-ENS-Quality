...
def main():
    ...
    args = ap.parse_args()
    result = {'tool': 'memory_retention', 'dry_run': args.dry_run}
    result.update({'status': 'retention_checked', 'policy': 'working=short, episodic=auditável, semantic=enquanto válido, procedural=review_required'})
    if args.report:

...
def main():
    ...
    args = ap.parse_args()
    result = {'tool': 'memory_capture', 'dry_run': args.dry_run}
    src = safe_path(args.input) if args.input else None
    result.update({'status': 'captured' if src else 'dry-run', 'input': args.input})
    if src and src.exists() and not args.dry_run:

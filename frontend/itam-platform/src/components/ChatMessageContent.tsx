    ...
    <>
      <div className="space-y-3 text-sm leading-relaxed">
        {parts.map((part: ChatMessagePart) => {
          ...
          if (part.type === "file") return <ChatFileCard key={part.id} part={part} role={role === "user" ? "user" : "assistant"} />;
        ...
        {textImagePreviews.map((part) => (
          <ChatFileCard key={`text-image-preview-${part.url}`} part={part} role={role === "user" ? "user" : "assistant"} />
        ))}

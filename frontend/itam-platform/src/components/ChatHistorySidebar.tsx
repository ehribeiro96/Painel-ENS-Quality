...
export const ChatHistorySidebar = ({
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (token) {
        await chatService.deleteSession(token, sessionToDelete);
      }
  ...
  const handleConfirmRename = async () => {
    ...
    try {
      ...
      if (token) {
        await chatService.renameSession(token, sessionToRename.id, newTitle.trim());
      }
  ...
  return (
    <>
      <div className="fixed left-0 top-0 h-screen w-64 glass-sidebar border-r border-white/10 flex flex-col z-40 bg-white/40 backdrop-blur-md transition-all duration-300 shadow-2xl md:shadow-none">
        {/* Header */}

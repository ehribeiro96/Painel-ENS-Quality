        ...
        <Base44Surface className="base44-operation-panel" as="section">
          ...
          {selectedUser ? (
            <Base44UserCard
              user={selectedUser}
              actions={
                <Base44StatusBadge status={selectedUser.status === "ACTIVE" ? "success" : selectedUser.status === "INACTIVE" ? "leitura" : "warning"}>
                  {selectedUser.status}
                </Base44StatusBadge>
              }
            />
          ...
          <div className="base44-operation-actions">
            <button className="button secondary" type="button" onClick={() => void copyPreview()} disabled={!preview}>
              Copiar HTML
            </button>
            <button className="button secondary" type="button" onClick={() => void downloadHtml()} disabled={!selectedUserId}>
              Baixar HTML
            </button>
            <a className="button secondary" href="/assinaturas/" target="_blank" rel="noreferrer">
              Abrir legado
            </a>
          </div>
        </Base44Surface>
        ...
        <Base44Surface className="base44-operation-panel" as="section">
          ...
          {preview ? (
            ...
            <Base44EmptyState
              ...
              description="Depois de gerar o preview, a assinatura aparecerá nesta área com a identidade Apoema."
            />
          )}
        </Base44Surface>
      </section>
